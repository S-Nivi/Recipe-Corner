...
document.getElementById("add_fav").addEventListener("click", async (event) => {
  if (user_info[4] == "NIL") {
    ...
    try {
      ...
      fav.setAttribute("src", "files/star_filled.png");
    } catch (error) {
      console.log("here1");
    ...
    try {
      ...
      user_info[4] = liked;
      localStorage.setItem("user_info", JSON.stringify(user_info));
      fav.setAttribute("src", "files/star_empty.png");
    } catch (error) {
      console.log("here1");
      console.error("Error adding user:", error);
    }
...
for (i in store) {
  if (store[i][0] == recipe) {
    ...
    imag.setAttribute("src", "files/" + store[i][3]);
    ...
    for (k = 0; k < rating; k++) {
      ...
      rate.setAttribute("src", "files/yellow_star.png");
      rate.className = "star";
      yellow_container.appendChild(rate);
    }

    if (user_info[4] != "NIL") {
      if (user_info[4].includes(recipe))
        fav.setAttribute("src", "files/star_filled.png");
      else fav.setAttribute("src", "files/star_empty.png");
    } else fav.setAttribute("src", "files/star_empty.png");
  }
}
